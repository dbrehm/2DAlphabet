...
with header.cd(projDir):
    if not options.skipScan: header.executeCmd('combine -M MultiDimFit -d card_%s.txt --algo grid --setParameterRanges r=%s,%s --cminDefaultMinimizerStrategy 0 --saveNLL -n %s -t %s --toysFile %s'%(tag,rrange[0],rrange[1],runname,options.toys,options.toysFile))
